...
const saveUserIdentity = async (userId, realmId, userInfo)=>{
  ...
  try {
    ...
    const { data, error } = await supabase.from('quickbooks_user_info').upsert({
      user_id: userId,
      realm_id: realmId,
  ...
  try {
    ...
    if (userIdentity) {
      try {
        ...
        const { data: existingIdentity } = await supabase.from('quickbooks_user_info').select('*').eq('user_id', userId).eq('realm_id', realmId).single();
        ...
        if (existingIdentity) {
          ...
          await supabase.from('quickbooks_user_info').update(identityInfo).eq('id', existingIdentity.id);
        } else {
          // Insert new record
          await supabase.from('quickbooks_user_info').insert(identityInfo);
        }
      } catch (error) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (path === 'authorize') {
      ...
      const { data, error } = await supabase.from('quickbooks_user_info').select('*').eq('user_id', userId).eq(realmId ? 'realm_id' : 'id', realmId || userId).maybeSingle();
      if (error) {
